...
def _(bbox_df, detection_metrics, mo, page_detection_metrics, parsing_models):
    ...
    page_det_df = page_detection_metrics("docbank", bbox_df, parsing_models)
    det_df = detection_metrics("docbank", bbox_df, parsing_models, per_page=page_det_df)
    mo.vstack([
...
def _(
    ...
    """
    cote_df = compute_cote_df("docbank", bbox_df, parsing_models)
...
def _(
    ...
    for _err, _err_label in [("spacer", "SpACER"), ("cdd", "CDD")]:
        ...
        latex_table(
            ...
            caption=rf"DocBank spearman correlation ($\rho$) between per-page $d_\text{{pars}}$ ({_err_label}) "
            ...
            label=f"tab:docbank_dpars_metrics_spearman_{_err}",
        )
...
def _(
    ...
    apply of jiwer_cer/spacer/cdd_decomp per OCR-model group — cheap per
    dataset here, but not free, and the combined cross-dataset validation
    notebook reads this cache directly rather than recomputing it. Delete
    the cache file to force a recompute after adding new OCR results.
    """
    _REPO_ROOT = Path(__file__).resolve().parent.parent
    _BOX_CACHE = _REPO_ROOT / "data/docbank/box_level_ocr_comparison.parquet"
