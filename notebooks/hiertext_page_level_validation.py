...
def _(mo):
    mo.md(r"""
    # HierText — page-level CER / SpACER / CDD validation

    Mirrors `notebooks/spiritualist_page_level_validation.py`. Reads
    `data/hiertext/page_level_cer_comparison.parquet`, produced by
...
@app.cell
def _(pd):
    page_values_df = pd.read_parquet("data/hiertext/page_level_cer_comparison.parquet")
...
def _(page_values_df):
    ...
    page_values_df.loc[(page_values_df['page']=='002a72617c3b1335') & (page_values_df['parsing_model']=='ppdoc_s')].round(3)
    return
...
def _(corr_df, p9):
    ...
    plot.save(filename='data/figures/hiertext_CEV_CER_correlation.pdf', dpi = 300)
    plot.draw()
    return (corr_long,)
