"""
Batch OCR processor using olmOCR via DeepInfra API — GT crop edition.
- Reads GT bounding boxes from a CSV (columns: filename, x, y, width, height, ssu_id)
- Crops each bbox from its source image and sends the crop to the DeepInfra olmOCR API
...
Usage:
    python scripts/olmocr_crop_deepinfra.py \\
        --bboxes     data/spiritualist/gt_ssu_bboxes.csv \\
        --input_dir  data/spiritualist/images \\
        --output     data/spiritualist/olmocr_crop_results.parquet
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Batch GT-crop OCR with olmOCR via DeepInfra")
    ...
    p.add_argument("--concurrency", default=4,    type=int,
                   help="Max simultaneous requests (default: 4)")
...
def main():
    ...
    for path, label in [(bboxes_path, "CSV"), (image_dir, "Image dir"), (env_path, ".env")]:
        if not path.exists():
