...
from scripts.run_ocr import load_model, process_image_group, merge_parts
...
def main():
    ...
    for ocr_model_name in args.ocr_models:
        ...
        for csv_path in csv_files:
            ...
            for filename in tqdm(remaining, desc=f"    {ocr_model_name}/{csv_path.stem}"):
                group_df = df[df["filename"] == filename].copy()
                try:
                    result_df = process_image_group(group_df, image_dir, model, args.batch_size)
